...
class Hospital{
  ...
  // resolveReferences used for link all related obj after load from JSON
    void resolveReferences(){
      ...
      for(var stay in stays){
        ...
        if(patient == null){
          print('Warning: Patient ID ${stay.patientId} not found for Stay ${stay.stayId}.');
          continue;
        }
        ...
        if (bed == null) {
        print('Warning: Bed ID ${stay.assignedBedId} not found for Stay ${stay.stayId}.');
        continue;
        }
        ...
        if(stay.isActive){
          ...
          print('Linked active Stay ${stay.stayId} to Bed ${bed.bedId} and Patient ${patient.name}.');
        }
      }
    ...
    List<Bed> get occupiedBeds => _bedMap.values.where((b) => b.status == BedAvailability.occupied).toList();
    ...
    void admitPatientToRoom({required Patient patient, required String roomNumber}){
    ...
    void dischargePatient({required String patientId}){
      ...
      if (activeStays.isEmpty) {
        throw Exception('Patient $patientId is not currently admitted (No active stay found).');
      }     
      ...
      if(bed == null || patient == null){
        throw Exception('Critical Data Error: Stay ${stay.stayId} has unlinked Patient or Bed.');
      }
  ...
  final room = rooms.firstWhere(
    ...
    orElse: () => throw Exception('Critical Data Error: Bed ${bed.bedId} not found in any room list.'),
  );
  
  print('--- Discharge Complete ---');
  print('Patient ${patient.name} discharged from Bed ${bed.bedId} in Room ${room.roomNumber}.');
